...
const LeetCodeWidget = () => {
    ...
    const fetchLeetCode = async () => {
      try {
        ...
        if (data.status === "success") {
          ...
          setStats({ error: data.message || "Failed to parse success status" });
        }
      } catch (error) {
        console.error("Error fetching LeetCode stats:", error);
        setStats({ error: error.message || "Network Error" });
      }
    };
    fetchLeetCode();
  }, []);
  ...
  if (!stats) {
    return (
      <div className="w-full glass-morphism rounded-[32px] p-6 border border-primary/10 flex flex-col items-center justify-center h-[200px] animate-pulse">
        <Code2 className="text-secondary/40 animate-spin mb-2" size={32} />
        <span className="text-sm font-mono text-secondary/60">Fetching from local proxy...</span>
      </div>
    );
  }
  ...
  if (stats.error) {
    return (
      <div className="w-full glass-morphism rounded-[32px] p-6 border border-rose-500/20 flex flex-col items-center justify-center h-[200px]">
        <Code2 className="text-rose-400 mb-2" size={32} />
        <span className="text-sm font-mono text-rose-400">Error: {stats.error}</span>
      </div>
    );
  }
